...
SNP_BY_COHORT = {
    "DREAM": "all_guanlab_snps",
    "BRAGGS": "all_guanlab_snps",
    "REACT": "all_guanlab_snps",
}
COHORT_MAP = {
    "DREAM": ["DREAM", "TEAR", "Immunex", "new"],
    "REACT": ["react"],
    "BRAGGS": ["BRAGGS", "EIRA", "ABCoN", "BRASS"],
}
PLOT_DICT = {
...
RESULTS_PATH = Path("results/drug_models/exp_5_mar2")
REGEX_PATTERNS = {
    "Full": r".",
...
MODEL_DICT = {
    ...
    "regressor": GaussianProcessRegressor(
        kernel=RBF(), n_restarts_optimizer=5, alpha=3
    ),
}
THRESHOLD = 2.5
SAMPLES = 10
...
def data_generator(path=Path(META_PATH), snp_dict=SNP_BY_COHORT, test_index=TEST_INDEX):

    # either create data or read from cache
    data = create_drug_df(path, snp_dict)
    engineered = feature_engineering(data)

    for cohort in snp_dict:

        for patt_name, pattern in REGEX_PATTERNS.items():
            _df = engineered[engineered["TNF-drug"] == cohort].reset_index(drop=True)
            _df["tt_split"] = ["train"] * test_index + ["test"] * (
                len(_df) - test_index
            )
            _df = _df[_df["Cohort"].isin(COHORT_MAP[cohort])]
            ...
            yield cohort, patt_name, X_train, X_test, y_train, y_test, cat_train, das_test, cat_test, yscaler, _df_x.columns
